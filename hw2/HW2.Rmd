...
for the observed data $\{(x_i, y_i)\}_{i=1}^n$.  
    ...
    (a) Show that the likelihood function $L(\beta_0, \beta_1)$ is always strictly less than 1.  
        ...
        Since $p(x_i)$ denotes the probability of the response of the $i^{th}$ observation, it can only be between 0 and 1. The likelihood function is the product of two cumulative product, and each value is between 0 and 1, so the final result will always in range of 0 and 1.
...
library(class)
set.seed(2018)
train.X = cbind(Lag1, Lag2, Lag3)[traingSet, ]
test.X = cbind(Lag1, Lag2, Lag3)[testSet, ]
train.Direction = traingSet$Direction
knn.pred = knn(train.X, test.X, train.Direction, k = 3)
table(knn.pred, testSet)
```
